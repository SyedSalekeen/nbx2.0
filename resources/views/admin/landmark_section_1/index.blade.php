...
@section('content')
    <!-- dashboard -->
    <div class="dashboard">
        <h3 class="text-[#8d2023] text-2xl font-semibold my-3"> Landmark Section First </h3>
        <!-- search and filters -->
        <div class="flex flex-wrap"> </div>
        <a href="{{ route('landmark_section_first_craete') }}"><button
            ...
            <tbody>
                @foreach ($landmarkSection as $key => $landmarkSec)
                    <tr>
                        ...
                        <td><a href="{{url('landmark_section-edit/'.$landmarkSec->id)}}">Edit</a> </td>
                    </tr>
                @endforeach
            </tbody>
        </table>
